...
Future<int> saveCity(CurrentWeatherModel model )async {
 ...
 +" VALUES(\"${model.location.name}\",\"${model.location.country}\",\"${model.location.time}\","
 +"${model.currentWeather.temperature},${model.currentWeather.wind_speed},${model.currentWeather.pressure},${model.currentWeather.humidity},\"${model.currentWeather.condition.climate}\",${model.currentWeather.condition.code},"
...
for(int i=0;i<list.length;i++){
  weather.add(new MainListRow(list[i]["name"],list[i]["temp"],list[i]["climate"],list[i]["climateCode"],list[i]["wind_speed"],list[i]["pressure"],list[i]["humidity"],
